  ...
  return (
    <div className="w-full max-w-md mx-auto space-y-5" dir="rtl">
      ...
      {isOutsideShiftWindow && !isCheckedIn && !isCheckedOut && (
        <div className="p-3.5 bg-amber-50 border border-amber-200 rounded-2xl text-amber-900 text-xs flex items-start gap-2.5 shadow-sm">
      ...
      <div className="flex items-center justify-between gap-2 px-4 py-2.5 bg-white border border-slate-200/80 rounded-2xl shadow-sm text-xs">
      ...
      <div className="relative p-6 bg-white border border-slate-200/80 rounded-3xl shadow-sm text-center space-y-6">
        {/* Shift Time Badge */}
        <div className="inline-flex items-center justify-center gap-2 px-3.5 py-1.5 bg-slate-100 rounded-xl text-xs md:text-sm font-medium text-slate-700">
          <span>⏰ {shiftName}:</span>
          <span dir="ltr" className="font-mono font-semibold text-slate-900">
            {scheduledStart} - {scheduledEnd}
          </span>
        </div>
          ...
          {isCheckedIn && !isCheckedOut && !isOnBreak && (
            <div className="w-full space-y-3">
              <Button
                ...
                className="bg-sky-600 hover:bg-sky-700 shadow-sky-600/20 text-base"
              >
                ☕ بدء استراحة مدفوعة
              </Button>

...
export default function Navbar({ user, notifications = [], onRefreshNotifications }: NavbarProps) {
  ...
  return (
    <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-md text-slate-900 border-b border-slate-200/80 shadow-sm">
      {/* Toast Notification العائم */}
      {toastNotif && (
        <div className="fixed top-16 left-4 right-4 sm:left-auto sm:right-6 sm:w-96 z-50 bg-white border border-sky-200 shadow-2xl rounded-2xl p-4 text-slate-900 animate-in slide-in-from-top-5 duration-300 flex items-start gap-3">
          ...
          <div className="flex-1 text-xs">
            <h4 className="font-bold text-slate-900 mb-0.5">{toastNotif.title}</h4>
            <p className="text-slate-600 leading-relaxed">{toastNotif.message}</p>
          </div>
        </div>
      )}
        ...
        <a href="/" className="flex items-center gap-2.5 group shrink-0">
          ...
          <div>
            <h1 className="text-sm font-bold tracking-tight text-slate-900 flex items-center gap-1.5">
              بصمة
              <span className="text-[9px] bg-sky-100 text-sky-700 font-semibold px-1.5 py-0.2 rounded-full border border-sky-200">
                v2.0.0
              </span>
            </h1>
            <p className="text-[10px] text-slate-500 hidden sm:block">Basma Enterprise v2.0.0</p>
          </div>
        </a>
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 py-1">
          {isAdminOrManager && (
            <div className="hidden md:flex items-center gap-1 shrink-0">
              <a
                ...
                className="px-2.5 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 rounded-xl text-xs font-semibold whitespace-nowrap transition-all border border-rose-200 flex items-center gap-1.5"
              >
              ...
              <a
                href="/admin/shifts"
                className="px-2.5 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-semibold whitespace-nowrap transition-all border border-slate-200"
              >
                <span>الورديات</span>
              </a>
              ...
              <a
                href="/admin/tasks"
                className="px-2.5 py-1.5 bg-slate-100 hover:bg-slate-200 text-amber-700 rounded-xl text-xs font-semibold whitespace-nowrap transition-all border border-slate-200"
              >
                <span>المهام</span>
              </a>
              ...
              <a
                ...
                className="px-2.5 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-700 rounded-xl text-xs font-semibold whitespace-nowrap transition-all border border-amber-200 flex items-center gap-1"
              >
              ...
              <a
                ...
                className="px-2.5 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 rounded-xl text-xs font-semibold whitespace-nowrap transition-all border border-emerald-200 flex items-center gap-1"
              >
              ...
              <a
                href="/admin"
                className="px-2.5 py-1.5 bg-sky-50 hover:bg-sky-100 text-sky-700 rounded-xl text-xs font-semibold whitespace-nowrap transition-all border border-sky-200 flex items-center gap-1"
              >
          ...
          <div className="hidden md:block relative shrink-0">
            <button
              ...
              className="p-1.5 text-slate-700 hover:text-slate-900 rounded-xl bg-slate-100 hover:bg-slate-200 border border-slate-200 relative"
            >
            ...
            {showDropdown && (
              <div className="absolute left-0 sm:right-auto mt-2 w-72 sm:w-96 bg-white border border-slate-200 rounded-2xl shadow-2xl p-3.5 z-50 text-xs animate-in fade-in zoom-in-95">
                  ...
                  {items.length > 0 ? (
                    items.map((item) => (
                      ...
                      >
                        ...
                        <p className="text-[11px] text-slate-600 mb-1">{item.message}</p>
          ...
          <div className="hidden lg:flex items-center gap-1.5 px-2.5 py-1 bg-slate-100 rounded-xl border border-slate-200 text-xs shrink-0">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
            <span className="text-slate-700 font-medium max-w-[120px] truncate">{user?.name || user?.email}</span>
            <span className="text-[9px] text-sky-700 bg-sky-100 px-1.5 py-0.5 rounded font-mono font-semibold">
              {user?.role}
            </span>
          </div>

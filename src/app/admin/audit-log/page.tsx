...
export default function AuditLogPage() {
  ...
  return (
    <div className="space-y-6 pb-20">
      {/* ترويسة سجل التدقيق */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white border border-slate-200/90 p-5 rounded-2xl shadow-sm transition-colors">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-slate-100 text-amber-600 flex items-center justify-center font-bold border border-slate-200 shrink-0">
        ...
        <button
          onClick={fetchLogs}
          disabled={loading}
          className="self-start sm:self-auto px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-lg flex items-center gap-2 transition-all border border-slate-200"
        >
      ...
      <div className="bg-white border border-slate-200 p-4 rounded-2xl shadow-sm space-y-4">
        <form onSubmit={handleSearchSubmit} className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pr-10 pl-4 py-2.5 bg-slate-50 border border-slate-300 rounded-lg text-xs text-slate-900 placeholder:text-slate-400 focus:bg-white focus:border-amber-500 focus:outline-none transition-colors"
            />
          </div>
          <button
            type="submit"
            className="px-5 py-2.5 bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs rounded-lg shadow-sm transition-colors shrink-0"
          >
            بحث
          </button>
        </form>
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none">
          <span className="text-xs font-bold text-slate-600 flex items-center gap-1 shrink-0 ml-2">
      ...
      <div className="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm">
        {loading ? (
          <div className="py-20 text-center space-y-3">
            ...
            <p className="text-xs font-bold text-slate-500">جاري تحميل سجل التدقيق الأمن المحصن...</p>
          </div>
        ) : logs.length === 0 ? (
          <div className="py-16 text-center space-y-3">
            <AlertCircle className="w-10 h-10 text-slate-400 mx-auto" />
            <p className="text-sm font-bold text-slate-700">لا توجد سجلات تدقيق تطابق معايير البحث الحالية</p>
          </div>
        ) : (
            ...
            {logs.map((log) => {
              ...
              return (
                <div key={log.id} className="relative group">
                  {/* عقدة الجدول الزمني */}
                  <div className="absolute -right-[31px] top-1.5 w-4 h-4 rounded-full bg-amber-500 ring-4 ring-white shadow-md" />

                  <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 transition-all hover:border-amber-500/40 space-y-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-xl bg-slate-200 flex items-center justify-center text-slate-700 text-xs font-bold">
                          <User className="w-4 h-4 text-amber-600" />
                        </div>
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-xs text-slate-900">{adminName}</span>
                            <span className="text-[10px] px-2 py-0.5 rounded-full bg-slate-200 text-slate-700 font-medium">
                              {log.user?.role || 'ADMIN'}
                            </span>
                          </div>
                          <p className="text-[11px] text-slate-500 mt-0.5 flex items-center gap-2">
                            <span>الكيان: <strong className="text-slate-700">{log.entity}</strong></span>
                            {log.ipAddress && (
                    ...
                    {(log.details || log.oldValue || log.newValue) && (
                      <div>
                        ...
                        {isExpanded && (
                          <div className="mt-3 p-3 bg-slate-50 text-slate-800 font-mono text-[11px] rounded-xl overflow-x-auto space-y-2 border border-slate-200">
                            {log.details && (
                              <div>
                                <span className="text-amber-700 font-bold">// التفاصيل الفنية (Details):</span>
                                <pre className="mt-1 dir-ltr text-slate-700">{JSON.stringify(log.details, null, 2)}</pre>
                              </div>
                            )}
                            {log.oldValue && (
                              <div>
                                <span className="text-rose-700 font-bold">// القيمة السابقة (Old Value):</span>
                                <pre className="mt-1 dir-ltr text-slate-700">{log.oldValue}</pre>
                              </div>
                            )}
                            {log.newValue && (
                              <div>
                                <span className="text-emerald-700 font-bold">// القيمة الجديدة (New Value):</span>
                                <pre className="mt-1 dir-ltr text-slate-700">{log.newValue}</pre>
                              </div>
                            )}
                          </div>
                        )}

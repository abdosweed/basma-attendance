  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white pb-20">
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        {/* ترويسة المركز */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/80 border border-slate-800 p-5 rounded-3xl backdrop-blur-md shadow-2xl">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h1 className="text-lg font-bold text-white flex items-center gap-2">
                ...
                {counts.total > 0 && (
                  <span className="bg-rose-500/20 text-rose-400 text-xs px-2.5 py-0.5 rounded-full border border-rose-500/30 font-bold animate-pulse">
                    {counts.total} طلب معلق
                  </span>
                )}
              </h1>
              <p className="text-xs text-slate-400 mt-0.5">اتخاذ القرارات المباشرة على طلبات الإجازات والاستئذان والتصحيح والأجهزة</p>
            </div>
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="self-start sm:self-auto px-4 py-2.5 bg-slate-800 hover:bg-slate-700 active:scale-95 text-slate-300 text-xs font-bold rounded-2xl flex items-center gap-2 transition-all border border-slate-700/80"
          >
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
            ...
            >
              <span>الكل</span>
              <span className={`px-2 py-0.5 rounded-full text-[10px] ${activeFilter === 'ALL' ? 'bg-white/20 text-white' : 'bg-slate-800 text-slate-300'}`}>
                {counts.total}
              </span>
            </button>
            ...
            >
              ...
              <span>الإجازات 📅</span>
              <span className={`px-2 py-0.5 rounded-full text-[10px] ${activeFilter === 'LEAVE' ? 'bg-white/20 text-white' : 'bg-slate-800 text-slate-300'}`}>
                {counts.leave}
              </span>
            </button>
            ...
            >
              ...
              <span className={`px-2 py-0.5 rounded-full text-[10px] ${activeFilter === 'HOURLY_PERMISSION' ? 'bg-white/20 text-white' : 'bg-slate-800 text-slate-300'}`}>
            ...
            >
              ...
              <span className={`px-2 py-0.5 rounded-full text-[10px] ${activeFilter === 'CORRECTION' ? 'bg-white/20 text-white' : 'bg-slate-800 text-slate-300'}`}>
                {counts.correction}
              </span>
            </button>
            ...
            >
              ...
              <span className={`px-2 py-0.5 rounded-full text-[10px] ${activeFilter === 'DEVICE' ? 'bg-white/20 text-white' : 'bg-slate-800 text-slate-300'}`}>
                {counts.device}
              </span>
            </button>
          </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pr-10 pl-4 py-2.5 bg-slate-900 border border-slate-800 rounded-2xl text-xs text-white placeholder-slate-500 outline-none focus:border-sky-500 transition-colors"
            />
          </div>
        </div>
        ...
        {loading ? (
          <div className="p-12 text-center bg-slate-900/50 border border-slate-800/80 rounded-3xl space-y-3">
            <RefreshCw className="w-8 h-8 text-sky-400 animate-spin mx-auto" />
            <p className="text-xs text-slate-400 font-bold">جاري جلب الطلبات المعلقة...</p>
          </div>
        ) : filteredItems.length === 0 ? (
          <div className="p-12 text-center bg-slate-900/50 border border-slate-800/80 rounded-3xl space-y-3">
            <CheckCircle2 className="w-12 h-12 text-emerald-400 mx-auto opacity-80" />
            <h3 className="text-sm font-bold text-white">لا توجد طلبات معلقة حالياً!</h3>
            <p className="text-xs text-slate-400">تم الانتهاء من مراجعة واعتماد جميع الطلبات بنجاح 🎉</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredItems.map((item) => (
              <div
                key={item.id}
                className="bg-slate-900 border border-slate-800 hover:border-slate-700/80 p-5 rounded-3xl shadow-xl flex flex-col justify-between transition-all space-y-4 animate-in fade-in duration-200"
              >
                {/* رأس البطاقة: بيانات الموظف والنوع */}
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <div className="w-11 h-11 rounded-2xl bg-slate-800 text-sky-400 font-bold flex items-center justify-center shrink-0 border border-slate-700/60 text-sm overflow-hidden">
                      {item.avatarUrl ? (
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-white leading-tight">{item.employeeName}</h4>
                ...
                <div className="bg-slate-950/70 border border-slate-800/80 p-3.5 rounded-2xl space-y-2 text-xs">
                  ...
                  {item.reason && (
                    <div className="flex items-start gap-2 text-slate-400 leading-relaxed border-t border-slate-800/60 pt-2">
                ...
                <div className="flex items-center gap-2 pt-1">
                  ...
                  <button
                    ...
                    disabled={processingId === item.id}
                    className="px-4 py-2.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 hover:text-rose-300 font-bold text-xs rounded-xl flex items-center justify-center gap-1.5 border border-rose-500/30 active:scale-95 transition-all"
                  >
                    <X className="w-4 h-4" />
                    <span>رفض</span>
                  </button>
      ...
      {rejectModalItem && (
        <div className="fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 w-full max-w-md shadow-2xl space-y-4 animate-in zoom-in-95 duration-200">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-300 mb-1.5">سبب الرفض (ملاحظة للموظف) *</label>
              <textarea
                required
                rows={3}
                value={rejectReason}
                ...
                className="w-full p-3 bg-slate-950 border border-slate-800 rounded-2xl text-xs text-white placeholder-slate-500 outline-none focus:border-rose-500 transition-colors"
              />
            </div>

            <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-800">
              <button onClick={() => setRejectModalItem(null)} className="px-4 py-2 bg-slate-800 text-slate-300 font-bold text-xs rounded-xl">
                إلغاء
              </button>
              <button

      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-900/80 border border-slate-800 rounded-3xl p-6 shadow-xl">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-white">طلبات وإدارة أجهزة الموظفين الموثوقة</h1>
          ...
          <button
            onClick={fetchData}
            disabled={loading}
            className="px-4 py-2.5 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 rounded-2xl text-xs font-bold transition-all flex items-center gap-2 self-end sm:self-center"
          >
        ...
        {statusMsg && (
          <div
            className={`p-4 rounded-2xl border text-xs font-medium flex items-center gap-2 shadow-lg transition-all ${
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div
            ...
            className={`cursor-pointer rounded-3xl p-5 space-y-2 border transition-all ${
              activeTab === 'REVOKED'
                ? 'bg-rose-500/10 border-rose-500/40 shadow-lg ring-1 ring-rose-500/30'
                : 'bg-slate-900/90 border-slate-800 hover:border-slate-700'
            }`}
          >
            <div className="flex items-center justify-between text-xs text-slate-400">
            ...
            <div className="text-3xl font-black text-rose-400">{counts.revoked}</div>
          ...
          <div
            ...
            className={`cursor-pointer rounded-3xl p-5 space-y-2 border transition-all ${
              activeTab === 'BLOCKED'
                ? 'bg-slate-800/80 border-slate-600 shadow-lg ring-1 ring-slate-500/30'
                : 'bg-slate-900/90 border-slate-800 hover:border-slate-700'
            }`}
          >
            <div className="flex items-center justify-between text-xs text-slate-400">
            ...
            <div className="text-3xl font-black text-slate-300">{counts.blocked}</div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-6 space-y-6 shadow-xl">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-800/80 pb-5">
            <div className="flex items-center gap-2 overflow-x-auto w-full sm:w-auto pb-2 sm:pb-0">
              ...
              >
                ...
                {counts.pending > 0 && (
                  <span className="px-2 py-0.5 bg-amber-500/30 text-amber-200 rounded-full text-[10px]">
                    {counts.pending}
                  </span>
                )}
              </button>
              ...
              >
                ...
                <span>الأجهزة المعتمدة</span>
                <span className="px-2 py-0.5 bg-emerald-500/30 text-emerald-200 rounded-full text-[10px]">
                  {counts.approved}
                </span>
              </button>
          ...
          {loading ? (
            <div className="p-12 text-center text-slate-400 flex flex-col items-center gap-3">
              ...
              <p className="text-xs">جاري تحميل أجهزة الفلتر المحدد...</p>
            </div>
          ) : filteredDevices.length === 0 ? (
            <div className="p-12 text-center text-slate-500 border border-slate-800/80 rounded-2xl bg-slate-950/40">
              <Smartphone className="w-12 h-12 text-slate-600 mx-auto mb-3" />
              <p className="text-sm font-bold text-slate-400">لا توجد أجهزة ضمن هذا التبويب حالياً.</p>
                    ...
                    return (
                      <tr key={dev.id} className="hover:bg-slate-800/40 transition-all">
                        <td className="p-4">
                          ...
                          <div className="text-[11px] text-sky-400 font-mono mt-0.5">#{emp?.employeeNumber}</div>
                        ...
                        <td className="p-4">
                          ...
                          <div className="text-[10px] text-slate-400 mt-1">
                            المنصة: <span className="text-slate-300">{dev.platform || 'PWA'}</span> | OS:{' '}
                            <span className="text-slate-300">{dev.os || 'Unknown'}</span> | المتصفح:{' '}
                            <span className="text-slate-300">{dev.browser || 'Unknown'}</span>
                          </div>
                        </td>

                        <td className="p-4 text-[11px] text-slate-400 space-y-1">
                          <div>الطلب: <span className="text-slate-300">{firstSeen}</span></div>
                          <div>آخر استخدام: <span className="text-slate-300">{lastSeen}</span></div>
                        </td>

                        <td className="p-4">
                          {dev.status === 'PENDING' && (
                            <span className="px-3 py-1 bg-amber-500/10 text-amber-400 border border-amber-500/20 rounded-full font-bold text-[11px] flex items-center gap-1 w-fit">
                          ...
                          {dev.status === 'BLOCKED' && (
                            <span className="px-3 py-1 bg-slate-800 text-slate-300 border border-slate-700 rounded-full font-bold text-[11px] flex items-center gap-1 w-fit">
                          ...
                          {dev.reviewNote && (
                            <p className="text-[10px] text-slate-400 mt-2 bg-slate-950 p-2 rounded-xl border border-slate-800/80">
                              💬 {dev.reviewNote}
                            </p>
                          )}
                        </td>

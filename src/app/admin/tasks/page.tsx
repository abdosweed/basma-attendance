    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-xs text-slate-400">جاري تحميل واجهة إدارة وتكليف المهام 2026...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col">
      ...
      <main className="flex-1 max-w-[98%] w-full mx-auto p-4 sm:p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {tasks.map((task) => (
            <div key={task.id} className="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 space-y-3">
              ...
              <div className="pt-3 border-t border-slate-800/80 space-y-2">
                <span className="text-[11px] font-bold text-slate-300 block">الموظفون المكلفون ({task.assignments?.length || 0}):</span>
                <div className="flex flex-wrap gap-1">
                  {task.assignments?.map((a: any) => (
                    <span
                      key={a.id}
                      className="text-[10px] font-bold px-2 py-0.5 bg-slate-950 text-slate-300 rounded-lg border border-slate-800 flex items-center gap-1"
                    >
          ...
          <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-slate-800 w-full max-w-xl rounded-3xl p-6 shadow-2xl space-y-5 animate-in zoom-in-95">
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                ...
                <button onClick={() => setShowTaskModal(false)} className="text-slate-400 hover:text-white p-1 rounded-lg">
                  ✕
                </button>
              </div>
              ...
              <form onSubmit={handleSaveTask} className="space-y-4 text-xs">
                <div>
                  <label className="block text-slate-400 mb-1 font-bold">عنوان المهمة</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-slate-400 mb-1 font-bold">تفاصيل ووصف المهمة</label>
                  <textarea
                    rows={3}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-slate-400 mb-1 font-bold">درجة الأهمية</label>
                    <select
                  ...
                  <div>
                    <label className="block text-slate-400 mb-1 font-bold">تاريخ التسليم المتوقع</label>
                    <input
                      type="date"
                ...
                <div>
                  <label className="block text-slate-400 mb-1.5 font-bold">الموظفون المكلفون بإنجاز المهمة</label>
                  <div className="max-h-40 overflow-y-auto border border-slate-800 rounded-xl p-2 space-y-1.5 bg-slate-950/60">
                    {employees.map((emp) => {
                      ...
                      return (
                        <div
                          ...
                          className={`p-2 rounded-lg border text-xs cursor-pointer flex items-center justify-between transition-all ${
                            isSelected ? 'bg-sky-500/20 border-sky-500/40 text-white font-bold' : 'bg-slate-900 border-slate-800 text-slate-300'
                          }`}
                        >
                ...
                <div className="pt-2 flex justify-end gap-2">
                  <button type="button" onClick={() => setShowTaskModal(false)} className="px-4 py-2 bg-slate-800 text-slate-300 rounded-xl">
                    إلغاء
                  </button>
